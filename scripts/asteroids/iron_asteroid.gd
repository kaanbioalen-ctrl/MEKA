...
## Iron asteroid visual controller.
## Applies a procedural shader to a Sprite2D child for all surface effects.
## Inherits all gameplay logic from AsteroidBase.
## _draw() handles only the death sequence;
## the shader handles the body, cracks, rim, and flicker.

const SHADER_RES := preload("res://shaders/iron_asteroid.gdshader")
const TEX_SIZE   := 128

const SCALE_MIN  := 0.90
const SCALE_MAX  := 1.10

const IRON_DEATH_DURATION: float = 0.35
...
func _update_death_visuals() -> void:
	if _mat == null or _sprite == null:
		return
	var t := clampf(1.0 - (_death_time_left / IRON_DEATH_DURATION), 0.0, 1.0)
	...
	if t < DEATH_FLASH_END:
		...
		_sprite.scale = Vector2(_base_scale * _scale_var * punch,
		...
		_sprite.scale = Vector2(_base_scale * _scale_var * body_s,
...
func _start_death() -> void:
	super._start_death()
	_death_time_left = IRON_DEATH_DURATION
	_spawn_death_shards()
	queue_redraw()
...
func _draw_death_burst() -> void:
	...
	if t >= DEATH_FLASH_END and t < DEATH_BREAK_END:
		...
		draw_arc(Vector2.ZERO, ring_r, 0.0, TAU, 48,
			Color(0.55, 0.53, 0.50, ring_a), 1.6, true)

	# Shards (phases 2 and 3)
	if t >= DEATH_FLASH_END and _shard_count > 0:
		var elapsed := (t - DEATH_FLASH_END) * IRON_DEATH_DURATION
		...
		for i in range(_shard_count):
			...
			draw_arc(pos, arc_r, ang - PI * 0.20, ang + PI * 0.20, 8,
				Color(0.76, 0.73, 0.68, shard_a), 1.8, true)
			if shard_a > 0.25:
				draw_circle(pos + Vector2(cos(ang), sin(ang)) * arc_r, 1.4,
					Color(0.92, 0.90, 0.86, shard_a * 0.7))
