...
class_name SulfurAsteroid

const SHADER_RES := preload("res://shaders/sulfur_asteroid.gdshader")
const TEX_SIZE := 128

const SCALE_MIN := 0.90
const SCALE_MAX := 1.10

const SULFUR_DEATH_DURATION: float = 0.35
...
func _update_death_visuals() -> void:
	if _mat == null or _sprite == null:
		return
	var t := clampf(1.0 - (_death_time_left / SULFUR_DEATH_DURATION), 0.0, 1.0)
...
func _start_death() -> void:
	super._start_death()
	_death_time_left = SULFUR_DEATH_DURATION
	_spawn_death_shards()
	queue_redraw()
...
func _draw_death_burst() -> void:
	...
	if t < DEATH_FLASH_END:
		...
		draw_arc(Vector2.ZERO, ring_r, 0.0, TAU, 64, Color(1.0, 0.94, 0.56, ring_a), 3.0, true)
		draw_circle(Vector2.ZERO, radius * lerpf(0.55, 0.20, pt), Color(1.0, 0.96, 0.72, ring_a * 0.65))

	if t >= DEATH_FLASH_END and t < DEATH_BREAK_END:
		var pt := (t - DEATH_FLASH_END) / (DEATH_BREAK_END - DEATH_FLASH_END)
		var ring_r := radius * lerpf(1.05, 2.4, pt)
		var ring_a := lerpf(0.45, 0.0, pt)
		draw_arc(Vector2.ZERO, ring_r, 0.0, TAU, 48, Color(0.92, 0.78, 0.16, ring_a), 1.6, true)

	if t >= DEATH_FLASH_END and _shard_count > 0:
		var elapsed := (t - DEATH_FLASH_END) * SULFUR_DEATH_DURATION
		...
		for i in range(_shard_count):
			...
			draw_arc(pos, arc_r, ang - PI * 0.20, ang + PI * 0.20, 8, Color(1.0, 0.88, 0.34, shard_a), 1.8, true)
			if shard_a > 0.25:
				draw_circle(pos + Vector2(cos(ang), sin(ang)) * arc_r, 1.4, Color(1.0, 0.97, 0.68, shard_a * 0.7))
